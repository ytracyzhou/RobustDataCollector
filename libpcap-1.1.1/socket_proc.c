...
int UpdatePacket(struct timeval * pTS, const u_char *sp, int dir, int bCooked) {
	...
	int protocol = (int)(*(sp+base+14+9));
	if (protocol != PROT_TCP && protocol != PROT_UDP) return PACKET_UNKNOWN_IP_PROT;	
	
	DWORD srcIP = *((DWORD *)(sp+base+14+12));
	DWORD dstIP = *((DWORD *)(sp+base+14+16));
	
	int offset = 4 * ((*(sp+base+14)) & 0x0F);
	int srcPort, dstPort;
	
	srcPort = (int)(*((WORD *)(sp+base+14+offset)));
	dstPort = (int)(*((WORD *)(sp+base+14+offset+2)));		
...
void StartCapture(char * pcapFilename) {	
	...
	sprintf(buf, "imap-%04d-%02d-%02d-%02d-%02d-%02d", 
		pTM->tm_year + 1900,	
		pTM->tm_mon + 1,
		pTM->tm_mday,
		pTM->tm_hour,
	...
	//create dir	
	/* tracyzhou begin */
	if (access("/sdcard/IMAP", 0) != 0) {
		if (mkdir("/sdcard/IMAP", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) != 0) {
			printf("Can not create directory /sdcard/IMAP\n");
			exit(0);
		} 
	}
	sprintf(pcapFilename, "/sdcard/IMAP/%s", buf);
	/* tracyzhou end */
	...
	//sync time
	/* tracyzhou begin */
	sprintf(pcapFilename, "/sdcard/IMAP/%s/time", buf);	
	/* tracyzhou end */
	printf("Synchronized timestamp file: %s\n", pcapFilename);
	FILE * ofs = fopen(pcapFilename, "w");	
	if (ofs == NULL) {
	...
	//event file
	/* tracyzhou begin */
	sprintf(pcapFilename, "/sdcard/IMAP/%s/processed_events", buf);
	/* tracyzhou end */
	...
	//appname file
	/* tracyzhou begin */
	sprintf(pcapFilename, "/sdcard/IMAP/%s/appname", buf);
	/* tracyzhou end */
	...
	/* tracyzhou begin */
	sprintf(pcapFilename, "/sdcard/IMAP/%s/traffic.cap", buf);
	/* tracyzhou end */
	printf("Pcap file: %s\n", pcapFilename);	
